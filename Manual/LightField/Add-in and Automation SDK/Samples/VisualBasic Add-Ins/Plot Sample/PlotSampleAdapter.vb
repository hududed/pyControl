...
<AddIn("Plot Sample (VB)", Version:="1.0.0", Publisher:="Teledyne Princeton Instruments", Description:="Demonstrates making data in memory and then plotting it as graphs" & vbCrLf & "It also demonstrates adding multiple graph sources to the same window")>
<QualificationData("IsSample", "True")>
Public Class PlotSampleAdapter
    Implements ILightFieldAddIn

    Private impl_ As PlotSample

    Public Sub Activate(ByVal lightField As ILightFieldApplication) Implements ILightFieldAddIn.Activate
        impl_ = New PlotSample
