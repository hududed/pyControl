...
<AddIn("Setting Snoop Sample (VB)", Version:="1.0.0", Publisher:="Teledyne Princeton Instruments", Description:="Demonstrates an add-in with no user interface that registers for setting changed events and logs them")>
<QualificationData("IsSample", "True")>
Public Class SettingSnoopSampleAdapter
    Implements ILightFieldAddIn

    Private impl_ As SettingSnoopSample

    Public Sub Activate(ByVal lightField As ILightFieldApplication) Implements ILightFieldAddIn.Activate
        impl_ = New SettingSnoopSample
